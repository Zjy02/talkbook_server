...
class UserController extends Controller {
  ...
  async login() {
    ...
    // app 是全局上下文中的一个属性，config / plugin.js 中挂载的插件，
    // 可以通过 app.xxx 获取到，如 app.mysql、app.jwt 等。
    // config / config.default.js 中抛出的属性，
    // 可以通过 app.config.xxx 获取到，如 app.config.jwt.secret。
    const { ctx, app } = this;
    const { username, password } = ctx.request.body;
